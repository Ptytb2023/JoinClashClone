...
namespace Model.Stickmen
{
	public class StickmanMovement
	{
		...
		private float _speedRightMove = 18f;

		public StickmanMovement(Entity model, SurfaceSliding surfaceSliding, InertialMovement inertialMovement,
			float distanceBetweenBounds)
		{
			Model = model;
		...
		public void MoveRight(float axis, float deltaTime)
		{
			Vector3 position = new Vector3
			{
				x = _speedRightMove * axis * deltaTime + Model.Position.x,
				y = Model.Position.y,
				z = Model.Position.z
			};
